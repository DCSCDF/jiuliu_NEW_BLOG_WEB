...
tool.data = {
    set(table, settings) {
        ...
        const BLOGSTRING = table.slice(0, 6) === 'BLOG_' && table !== 'BLOG_SYS_BASE_CONFIG'
        if (BLOGSTRING) {
            let localSetting = JSON.parse(localStorage.getItem('BLOG_SETTING')) || {}
            let newSetting = {}
            newSetting[table] = _set
            return localStorage.setItem('BLOG_SETTING', JSON.stringify(Object.assign(localSetting, newSetting)))
        } else return localStorage.setItem(table, _set)
    },
    get(table) {
        const BLOGSTRING = table.slice(0, 6) === 'BLOG_' && table !== 'BLOG_SYS_BASE_CONFIG'
        const BLOG_SETTING = JSON.parse(localStorage.getItem('BLOG_SETTING')) || {}
        let data = BLOGSTRING ? BLOG_SETTING[table] : localStorage.getItem(table)
        try {
